...
const Recommendations = async () => {
  ...
  const friendRecommndations = await prisma.user.findMany({
    where: {
  ...
  return (
    <div className="p-4 rounded-2xl border-[1px] border-borderGray flex flex-col gap-4">
      {friendRecommndations.map((person)=>(
      <div className="flex items-center justify-between" key={person.id}>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="">
            <h1 className="text-md font-bold">{person.username||person.displayName}</h1>
